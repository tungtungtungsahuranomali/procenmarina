...
//var site_url = "http://192.168.120.201/iptvman_p/";
var site_url = "";
...
//                                SAMPAI SINI
				var run = $('table tr#'+escape_device_id+' > td:nth-child(2) > input[type="text"][name="run-'+escape_device_id+'"]').val();
				var rep = $('table tr#'+escape_device_id+' > td:nth-child(3) > input[type="number"][name="rep-'+escape_device_id+'"]').val();
				var tim = $('table tr#'+escape_device_id+' > td:nth-child(4) > input[type="number"][name="tim-'+escape_device_id+'"]').val();
				var o = {};
				o[device_id] = {r_text : run, repetition : rep, r_time  : tim};
				guest_data.post_data.push(o);
				
			});
			$.post(site_url+'r-text/updating',  guest_data, function(data){
				if (data === "true"){
					//alert("BERHASIL");
					$('#myModal').modal('hide');
					//location.reload();
	...
	$('.btn-reset').click(function (e) {
		...
		$('#confirmation-vacant-yes').off('click').on('click', function(){
			...
			$.post(site_url+'r-text/resetting',  device_data, function(data){
				if (data === "true"){
					//alert("BERHASIL");
					$('#myView').modal('hide');
					location.reload();
...
//					dateFormat: 'yy-mm-dd'
//				 });
				$('[class*=btn-update-]').click(function (e) {
					e.preventDefault();
...
//                                        console.log('table tr#'+escape_device_id+' > td:nth-child(2) > input');
				...
				$('[class*=btn-reset-]').click(function (e) {
					e.preventDefault();
					var arr = $(this).attr('class').split('-');
					var device_id = arr[3];
					 // var device_id = $(this).attr('class').slice(-13);
