...
$takedatacsr = "SELECT * FROM information WHERE id_information = '$getidcsr'";
$connettakedatacsr = mysqli_query($koneksi, $takedatacsr);
while($takedata = mysqli_fetch_array($connettakedatacsr)){
  $judulcsr = $takedata['judul'];
  $deskripsicsr = $takedata['deskripsi'];
  //$gambaremergency = $takedata['gambar'];
  $kategoricsr = $takedata['kategori'];
}

if(isset($_POST['submit'])){
  $judulcsr1 = $_POST['judulcsr'];
  $deskripsicsr1 = $_POST['deskripsicsr'];
  ...
  if(!empty(trim($judulcsr1))){
    if(!empty(trim($deskripsicsr1))){

        if($connettakedatacsr){
          $insertdata = "UPDATE information set judul = '$judulcsr1', deskripsi = '$deskripsicsr1' WHERE id_information = '$getidcsr'";
  ...
  if($usersession){
    ...
    <div class="wrapper d-flex align-items-stretch">
      ...
      <div id="content" class="p-4 p-md-5">
        ...
        <h2 class="mb-4"><?php echo $judulcsr; ?><Title></Title></h2>

         <form method="POST" enctype="multipart/form-data">
          <div class="mb-3 w-75">
            <label for="exampleFormControlInput2" class="form-label">Judul</label>
            <input type="text" class="form-control" name="judulcsr" id="exampleFormControlInput2" value ="<?php echo $judulcsr; ?>">
          </div>
          ...
          <div class="mb-3 w-75" style="display: flex; align-items: center;">
            ...
            <textarea name="deskripsicsr" style="width: 450px; height: 200px;"><?php echo $deskripsicsr; ?></textarea>
          </div>
