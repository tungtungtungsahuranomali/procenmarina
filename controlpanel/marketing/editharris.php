...
$takedataharris = "SELECT * FROM about WHERE id_about = '$getidhotel'";
$connettakedataharris = mysqli_query($koneksi, $takedataharris);
while($takedata = mysqli_fetch_array($connettakedataharris)){
...
if(isset($_POST['submit'])){
  $judulharris = $_POST['judulharris'];
  $deskripsiharris = $_POST['deskripsiharris'];
  $gambarharris = $_FILES['gambarharris']['name'];
  $targetpath = "images/picabout/";
  $path = $targetpath.basename($gambarharris);
  $tmpgambar = $_FILES['gambarharris']['tmp_name'];

  

  if(!empty(trim($judulharris))){
    if(!empty(trim($deskripsiharris))){
      if(move_uploaded_file($tmpgambar, $path)){
        $insertdata = "UPDATE about set judul_about = '$judulharris', deskripsi_about = '$deskripsiharris', gambar_about = 'http://172.31.15.253/controlpanel/images/picabout/$gambarharris' WHERE id_about = '$getidhotel'";
        ...
        <h2 class="mb-4"><?php echo $namahotel; ?><Title></Title></h2>
         ...
         <form method="GET" enctype="multipart/form-data">
          <div class="mb-3 w-75">
            ...
            <input type="text" class="form-control" name="judulharris" id="exampleFormControlInput2" value="<?php echo $judulabout; ?>">
          </div>
          ...
          <div class="mb-3 w-75" style="display: flex; align-items: center;">
            ...
            <textarea name="deskripsiharris" style="width: 450px; height: 200px;"><?php echo $deskripsiabout; ?></textarea>
          </div>

          <div style="display: flex; flex-direction: column;">
            <input type="file" name="gambarharris">
          </div>
